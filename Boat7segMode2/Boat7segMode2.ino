...
bool Started = false;  // check if we are in a countdown
double StartedAt = 0; // time that we started the countddown at
...
//Two Buttons added
int BR = A2; //A2 is red button
int BY = A3; //A3 is yellow button
...
{
  if (Started) {
    // we are already started so display the count down, wait for stop or sync
    message = "  4:59  ";

    //read the red to see if stopped
    int Red = digitalRead(BR);
    //if stopped, set to not started
    if (!Red) {
      Started = false;
    }
    // read the yellow to see if synchro
    int Yellow = digitalRead(BY);
    // move to next minute
    if (!Yellow) {
      message = "  3:59  ";
    }
    ...
    // we are not started so set to 5:00 to go and wait for start button
    message = "  5:00  ";

    //read the red, grab the real time and set to started
    int Red = digitalRead(BR);
    if (!Red) {
      Started = true;
    }
...
void setup()
{
  ...
  pinMode(MODEPIN1, INPUT_PULLUP); // D2 Land or sea
  pinMode(MODEPIN2, INPUT_PULLUP); // D3 DST auto or manual
  pinMode(MODEPIN3, INPUT_PULLUP); // D4 DST On or off when manual

  pinMode(LEDPIN1, OUTPUT); //D5
  pinMode(LEDPIN2, OUTPUT); //D6
  pinMode(DisplayCS, OUTPUT);
  pinMode(Display2CS, OUTPUT);
  pinMode(BR, INPUT_PULLUP); // Red button
  pinMode(BY, INPUT_PULLUP); // Yellow button
