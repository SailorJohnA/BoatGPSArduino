...
int sensorValue = 1;
int sensorValue1 = 1;
int K0 = 0;  //Knob 0 is pin A0
int K1 = 1;  //Knob 1 is pin A1
int OldKnob = 5;
int oldvalue1 = 1;
int newvalue1 = 1;
int OldKnob1 = 5;
...
//Two Buttons added
int ButRed = 2; //    2 is red button Interrupt
int ButYellow = 3; // 3 is yellow button Interrupt
volatile bool NotStarted = true; // Not yet stated
volatile bool Running = false;  //  started
volatile bool JustRed = false; //Red was pushed
volatile bool JustYellow = false; // Yellow was pushed


int StartTypeTimeMins = 1; // How long is the start timer to show
unsigned long StartMillis = 0;
unsigned long StoppedMillis = 0;
int countdown = 0;  // the time to show
int elapsed = 0;
...
const int MODEPIN1 = A2;
const int MODEPIN2 = A3;
const int MODEPIN3 = A4;
const int LEDPIN1 = 5;
const int LEDPIN2 = 6;
...
void ShowVers() // Show softwre level
{
  message =  "04.1    2"; // SW , HW version
  to7seg(message, 1);// 2x8 Digit 7 segs is type 2 HW
  //two knobs, two buttons and a three SPST DIP switch
  message =  "30-09-19" ;
  to7seg(message, 2);
  delay(2000);
}
static void doSomeWork()
{
  ...
  SetBrightness();
  //DoDusk();
  if (digitalRead(MODEPIN1)) // in sea mode
  {
    switch (checkmode2()) {
      case 0:
        ShowTime(1);
...
{ if (                     avgvalue(K0) < 166)    {
  ...
  if (avgvalue(K0) >= 166 && avgvalue(K0) < 332)    {
    return 1;
  }
  if (avgvalue(K0) >= 332 && avgvalue(K0) < 478)    {
    return 2;
  }
  if (avgvalue(K0) >= 478 && avgvalue(K0) < 642)    {
    return 3;
  }
  if (avgvalue(K0) >= 642 && avgvalue(K0) < 808)    {
    return 4;
  }
  if (avgvalue(K0) >= 808 )   {
    return 5;
...
void Start(int unit)
{
  ...
  int count = ((StartTypeTimeMins * 60) - elapsed);
  DisplayTime(count, unit);
}

void NotYetStarted()
{
  Serial.print(" not yet started ");
  elapsed = 0;
  if (JustRed) {
    JustRed = false;
    Running = true;
...
void CountDown()
{
  ...
  int count = (millis() - StartMillis) / 1000;
  if (count = StartTypeTimeMins * 60 - 1) StartMillis = millis();
  elapsed = count;
  if (JustRed) {
    Running = false;
    JustRed = false;
...
void DoSynch()
{
  //int count = count - 60000;
  Serial.println (" Synching ");
}
void Stopped()
{
  Serial.print("Stopped");
  int count = (StoppedMillis - StartMillis) / 1000;
  if (JustYellow) {
    JustYellow = false;
    NotStarted = true;
  }
...
void DisplayTime (int thecount, int unit)
{
  int themins;
  int thesecs;
  String showmins;
  String showsecs;
  String showstart = String(StartTypeTimeMins) + (" ");
  String showend = "  ";
  themins = (abs(thecount / 60));
  thesecs = thecount - abs(thecount / 60);

  if (themins < 10) showmins = (" 0.");
  else showmins = String(themins);

  if (thesecs < 10) showsecs = String("0") + String(thesecs);
  else showsecs = String(thesecs);

  Serial.print( " count is "); Serial.println(thecount);

  message = showstart + showmins + showsecs + showend;
  to7seg(message, unit);
}
void RedPush()
{
...
void setup()
{
  ...
  pinMode(MODEPIN1, INPUT_PULLUP); //  Land or sea
  pinMode(MODEPIN2, INPUT_PULLUP); //  DST auto or manual
  pinMode(MODEPIN3, INPUT_PULLUP); //  DST On or off when manual

  pinMode(LEDPIN1, OUTPUT); //D5
  pinMode(LEDPIN2, OUTPUT); //D6
  pinMode(DisplayCS, OUTPUT);
  pinMode(Display2CS, OUTPUT);
  pinMode(ButRed, INPUT_PULLUP); // Red button D2 interrupt
  pinMode(ButYellow, INPUT_PULLUP); // Yellow button D3 interrupt

  attachInterrupt(digitalPinToInterrupt(ButRed), RedPush, LOW);
  attachInterrupt(digitalPinToInterrupt(ButYellow), YellowPush, LOW);
